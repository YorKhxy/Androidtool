...
| 功能点 | 描述 | 优先级 |
|--------|------|--------|
| Logcat 全局日志 | 抓取系统所有日志（`adb logcat -v threadtime`，解析 PID/TID/TAG/级别） | P0 |
...
| 多行日志合并 | 异常堆栈等单条日志在 logcat 中是多条「同 时间+PID+TID+级别+TAG」的物理行，合并为一条日志条目（message 多行）。过滤/搜索按整条匹配——只要堆栈中任一行命中关键字，整条（含异常头与全部调用栈）都保留，与 Android Studio 一致，避免只剩零散命中行而丢失上下文。列表中整条铺开显示全部行（采用变高虚拟滚动，行高按文本行数确定性计算）；过长单行横向裁切，完整内容可在详情面板查看 | P0 |
