...
- **恢复触发 = 启动即弹窗**：应用启动时读 journal，若发现有未完成（仅崩溃 / 被杀残留的）任务，弹窗提示"上次有 N 个文件未传完，是否继续 / 丢弃"，由用户决定。
- **恢复以原设备为前提**：未完成任务绑定原 `deviceId`。恢复时若原设备未连接，提示"等待设备 XXX 连接"，**只有原设备重新连上才能继续**，不允许改投到当前其他设备（避免传错设备 / 路径不存在）。
