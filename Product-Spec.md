...
| 声音去向实时切换 | 投屏时可勾选「把设备声音传到电脑」，**默认关闭**（声音留在设备本机输出）。**投屏过程中可随时切换**：主视频窗口永远 `--no-audio`，音频由一个独立的「纯音频」scrcpy 进程（`--no-video --no-control --no-window`）承载，勾选即起该进程把声音转到电脑（设备静音）、取消即停掉该进程让声音回到设备——切换不影响、不闪动主画面 | P1 |
