...
1.5. **更新说明（自动）**：`make-update-package` 会**从 git 提交自动生成**本次更新说明（取上次发版以来的 feat/fix/perf 提交、去前缀）写进 `release-notes.md`，再打进 `latest.yml`。朋友热更后会在「发现新版本 / 已就绪」提示里看到「本次更新说明」。**不用手写**。想手写时给脚本加 `-NoAutoNotes`，改 `release-notes.md` 后再打包。
