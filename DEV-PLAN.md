...
**状态**：已完成（分支 hxy0601；Task 14.1–14.7 全部提交，build + 44/44 test 通过、code-review 两阶段过；视频跨段拖动连续性、画面/游标同步、Pico 单眼截图等待真机回归）
...
**Task 14.6 — 参数过滤打标记**
- [ ] 过滤控件：报告区新增过滤面板——选指标（FPS / CPU / MEM / GPU，GPU 仅 Pico）、运算符（`>` / `=` / `<`）、阈值输入；支持添加多条件，**每条件各自独立标记，不做 AND 交集**（对齐用户 2026-06-04 修订）。阈值留空给「不能为空」提示。
- [ ] 命中标记：点「过滤」对样本序列逐点求值，**每条件各自把命中点标在「该指标自己的曲线」上、按指标颜色区分、跟随该曲线显隐**（隐藏某曲线时其过滤标记一并隐藏；区别于波峰波谷三角，用空心圆点），并把结果整理为 `PerformanceCaptureMarker[]` 经 `SAVE_CAPTURE_MARKERS` 持久化到会话。
...
- 选指标配阈值（>/=/<，多条件各自独立标在对应指标曲线、按色区分、跟随曲线显隐）点过滤，曲线出现命中标记；单击标记时间轴与视频跳转到对应位置并暂停
...
| Phase 14: 性能模块重构 | 已完成 | 删快照链路、持续分段录制、采集会话归档、报告时间轴联动、参数过滤打标记、回看列表加载/删除、视频快捷截图；取代 Phase 4 快照/短录制（待真机回归） |
| Phase 15: Pico 弱网控制桌面集成 | 待开发 | 「弱网」标签页、内置助手 APK 一键安装、目标包名选择、预设档位+手动参数、START/STOP 下发、tun 地址状态查询、VPN 授权引导；新增 4 个弱网 IPC（三处同步 + 双 entry 注册），助手端 APK 已完成 |
